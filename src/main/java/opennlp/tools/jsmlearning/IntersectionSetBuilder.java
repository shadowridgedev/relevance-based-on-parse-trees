...
public class IntersectionSetBuilder {
	...
	// The set of attribute in analysis is hard coded
	String[] fieldsToAggr = new String[] { "reason_code", "risk_rating", "service_type", "device_match_result",
			"device_result", "http_referer", "device_id_reason_code", "review_status", "tcp_os_sig_ttl",
			"tcp_connection_type", "mime_type_number", "plugin_number", "http_connection_type", "device_last_event",
	...
	public void ruleVerifier(String dataFile, String ruleFile) {
		...
		List<Map<String, String>> rules = new ArrayList<Map<String, String>>(),
				dedupedRules = new ArrayList<Map<String, String>>();
		...
		for (Map<String, String> rule : rules) {
			...
			output.add(new String[] { rule.toString(), new Integer(countCoverPos).toString(),
	...
	private List<Map<String, String>> formIntersections(List<Map<String, String>> intersectionsIn,
		...
		for (int i = 0; i < intersectionsIn.size(); i++) {
			for (int j = i + 1; j < intersectionsIn.size(); j++) {
				Map<String, String> intersection = distProcessorNeg.computeIntersection(intersectionsIn.get(i),
	...
	private List<Map<String, String>> formIntersectionAmongMembersOfTrainingSetAndVerifyThatDoesNotCoverOppositeTrainingS(
		...
		for (int i = 0; i < negativeSet.size() && i < 1000; i++) {
			for (int j = i + 1; j < negativeSet.size(); j++) {
				Map<String, String> intersection = distProcessorNeg.computeIntersection(negativeSet.get(i),
						negativeSet.get(j));
				if (intersection.isEmpty())
					continue;
				 ...
				 * countCoverNeg++; }
				 * 
				 * } if (countCoverNeg<2){ System.err.
				 * println("A rule formed but it does not cover its origin! "
	...
	private List<Map<String, String>> filterIntersectionsByOppositeTrainingSet(List<Map<String, String>> intersections,
	...
	public static void main(String[] args) {
		...
		String resFile = iBuilder
				.ruleFormer("C:/workspace/relevanceEngine/src/test/resources/maps/anomaly/negativeSet1.csv");
...
/*
 * 
 * datetime browser_language browser_string device_first_seen
 * device_match_result http_os_signature http_os_sig_raw os
 * device_id_reason_code true_ip proxy_ip http_os_sig_adv_mss
 * http_os_sig_snd_mss http_os_sig_rcv_mss http_os_sig_ttl http_connection_type
 * device_last_event flash_lang flash_os flash_version os_fonts_number
 * plugin_adobe_acrobat plugin_flash plugin_silverlight
 * plugin_windows_media_player profiling_datetime screen_res tcp_os_signature
 * tcp_os_sig_raw time_zone time_zone_dst_offset profile_api_timedelta
 * mime_type_number plugin_number plugin_quicktime plugin_java
 * fuzzy_device_id_confidence fuzzy_device_match_result fuzzy_device_last_event
 * fuzzy_device_first_seen true_ip_city true_ip_first_seen true_ip_geo
 * true_ip_latitude true_ip_longitude account_email_first_seen
 * shipping_address_first_seen tcp_os_ sig_ttl tcp_connection_type page_time_on
 * policy_score reason_code review_status risk_rating
 */
