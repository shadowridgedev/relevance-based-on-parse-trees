...
public class JSMLearnerOnLatticeBase {
	...
	public JSMDecision buildLearningModel(List<String> posTexts, List<String> negTexts, String unknown,
		...
		if (separationKeywords != null) { // re-sort by occurrence of separation
											// keyword
		...
		for (int iConcept = 0; iConcept < psPos.conceptList.size(); iConcept++) {
			...
			if (psNeg.conceptList.get(iConcept).intent != null && psNeg.conceptList.get(iConcept).intent.size() > 0) {
				intersection = md.matchTwoSentencesGroupedChunksDeterministic(psNeg.conceptList.get(iConcept).intent,
						chunksUnknown);
				if (reduceList(intersection).size() > 0)
		...
		for (int iConcept = 0; iConcept < psPos.conceptList.size(); iConcept++) {
			for (int iConceptJ = 0; iConceptJ < posIntersections.size(); iConceptJ++) {
				intersection = md.matchTwoSentencesGroupedChunksDeterministic(psPos.conceptList.get(iConcept).intent,
		...
		JSMDecision decision = new JSMDecision("keywordClassName", bPositiveClass, posIntersections, negIntersections,
				posIntersectionsUnderNeg, negIntersectionsUnderPos, separationKeywords);
	...
	private List<List<ParseTreeChunk>> computeIntersectionWithIntentExtendedByDeduction(
		...
		return md.matchTwoSentencesGroupedChunksDeterministic(psPos.conceptList.get(iConcept).intent, chunksUnknown);
	...
	public Pair<List<List<List<ParseTreeChunk>>>, List<List<List<ParseTreeChunk>>>> removeInconsistenciesFromPosNegIntersections(
		...
		/*
		 * System.out.println("pos = "+ pos); System.out.println("neg = "+ neg);
		...
		return new Pair<List<List<List<ParseTreeChunk>>>, List<List<List<ParseTreeChunk>>>>(cleanedFromInconsPos,
				cleanedFromInconsNeg);
	}
	...
	public static void main(String[] args) {
		...
		JSMDecision dec = new JSMLearnerOnLatticeBase().buildLearningModel(Arrays.asList(posArr), Arrays.asList(negArr),
				unknown, null);
