...
public class FeatureSpaceCoverageProcessor {
	...
	public Float calcDistance(String[][] seed, String[] candidate) throws Exception {
		...
		for (int v = 0; v < seed[0].length; v++) {
			try {
				int p3 = paramMap.get("Latitude");
				int p4 = paramMap.get("Longitude");
				if (seed[p3][v].equals("") || seed[p4][v].equals("") || candidate[p3].equals("")
						|| candidate[p4].equals(""))
					continue;
	...
	public Map<String, String> computeIntersection(String[] line1, String[] line2) {
		...
		for (String attr : attributes) {
			...
			if (valArr1Str == null || valArr2Str == null) { // we assume single
															// value, not an
															// array of values
				if (v1.equals(v2)) {
				...
				/*
				 * verification of coverage valList1.retainAll(valList2);
				 * 
				 * List<String> vl1 = new
				 * ArrayList<String>(Arrays.asList(valArr1));
	...
	public boolean ruleCoversCase(Map<String, String> attr_value, String[] line) {
		...
		for (String attr : attributes) {
			...
			if (vCase == null) {// rule for this attribute exists but case has
								// no value
				soFarCovers = false;
				return false;
			}
			...
			if (valArrCaseStr == null || valArrRuleStr == null) { // we assume
																	// single
																	// value,
																	// not an
																	// array of
	...
	public boolean ruleCoversRule(Map<String, String> attr_value, Map<String, String> line) {
		...
		for (String attr : attributes) {
			...
			if (vRuleBeingCovered == null) {// rule for this attribute exists
											// but RuleBeingCovered has no value
				soFarCovers = false;
				return false;
			}
			...
			String valArrRuleStr = StringUtils.substringBetween(rule, "{", "}");
			if (valArrRuleBeingCoveredStr == null || valArrRuleStr == null) { // we
																				// assume
																				// single
																				// value,
																				// not
