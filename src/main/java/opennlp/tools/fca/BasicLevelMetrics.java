...
public class BasicLevelMetrics {
	...
	public double[] simJ_SMC(ArrayList<Integer> intent1, ArrayList<Integer> intent2) {
		...
		simSMC = (this.attributes.size() > 0) ? 1. * (intersection.size() + unionOut.size()) / this.attributes.size()
				: 0;
	...
	public void similarityGoguenNorm() {
		...
		for (FormalConcept c : cl.conceptList) {
			...
			c.blS_SMCaa = (c.cohAvgSMC != 0)
					? c.cohAvgSMC * upperCohAvgByAvgSMC(c, tetta) * lowerCohAvgByAvgSMC(c, tetta) : 0;
			c.blS_SMCma = (c.cohMinSMC != 0)
					? c.cohMinSMC * upperCohAvgByMinSMC(c, tetta) * lowerCohAvgByMinSMC(c, tetta) : 0;
			c.blS_SMCam = (c.cohAvgSMC != 0)
					? c.cohAvgSMC * upperCohMinByAvgSMC(c, tetta) * lowerCohMinByAvgSMC(c, tetta) : 0;
			c.blS_SMCmm = (c.cohMinSMC != 0)
					? c.cohMinSMC * upperCohMinByMinSMC(c, tetta) * lowerCohMinByMinSMC(c, tetta) : 0;
		}
	}
