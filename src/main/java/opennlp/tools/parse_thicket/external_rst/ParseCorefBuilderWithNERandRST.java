...
public class ParseCorefBuilderWithNERandRST {
	...
	// creates a list of Arcs objects 'arcs' from the descourse tree dt, using
	// the list of sentences 'nodesThicket' to identify words
	...
	private void navigateDiscourseTree(DiscourseTree dt, List<WordWordInterSentenceRelationArc> arcs,
		...
		if (dt.isTerminal()) {
			...
			ArcType arcType = new ArcType("rst", dt.relationLabel() + "=>" + dt.kind(),
			...
			WordWordInterSentenceRelationArc arc = new WordWordInterSentenceRelationArc(
					new Pair<Integer, Integer>(dt.firstToken().copy$default$1(), dt.firstToken().copy$default$2()),
					new Pair<Integer, Integer>(dt.lastToken().copy$default$1(), dt.lastToken().copy$default$2()),
					lemmaFrom, lemmaTo, arcType);
