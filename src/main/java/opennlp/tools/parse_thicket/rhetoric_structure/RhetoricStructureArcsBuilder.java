...
public class RhetoricStructureArcsBuilder {
	...
	private Map<Integer, List<Pair<String, Integer[]>>> buildMarkers(ParseThicket pt) {
		...
		Map<Integer, List<Pair<String, Integer[]>>> sentNumMarkers = new HashMap<Integer, List<Pair<String, Integer[]>>>();
		int count = 0;
	 ...
	 * 
	 * Input: obtained RST markers (numbers of words which splits sentence in
	 * potential RST relation arguments) + Current Parse Thicket with arcs for
	 * coreferences
	 * 
	 * We search for parts of sentences on the opposite side of RST markers
	 * 
	 * $sentPosFrom$ marker | == == == [ ] == == == | \ \ \ \ coref RST arc
	 * being formed \ \ \ \ | == == == == == [ ] == == ==|
	 * 
	 * Mark yelled at his dog, but it disobeyed | \ coref RST arc for CONTRAST
	 * being formed | \ He was upset, however he did not show it $sentPosTo$
	 */
		...
		for (int nSentFrom = 0; nSentFrom < pt.getSentences().size(); nSentFrom++) {
			for (int nSentTo = nSentFrom + 1; nSentTo < pt.getSentences().size(); nSentTo++) {
				...
				for (WordWordInterSentenceRelationArc arc : arcs) {
					...
					if (nSentFrom != arc.getCodeFrom().getFirst() || nSentTo != arc.getCodeTo().getFirst()
						...
						for (List<ParseTreeNode> vpTo : phrasesTo) {
							for (Pair<String, Integer[]> mFrom : markersFrom) {
								for (Pair<String, Integer[]> mTo : markersTo) {
									{
										// the phrases should be on an opposite
										// side of rst marker for a coref link
										if (isSequence(new Integer[] { sentPosFrom, vpFrom.get(0).getId(),
												mFrom.getSecond()[0] })
												&& isSequence(new Integer[] { sentPosTo, vpTo.get(0).getId(),
														mTo.getSecond()[0] })) {
											...
											WordWordInterSentenceRelationArc arcRST = new WordWordInterSentenceRelationArc(
													new Pair<Integer, Integer>(nSentFrom, mFrom.getSecond()[1]),
													new Pair<Integer, Integer>(nSentTo, mTo.getSecond()[1]), "", "",
													arcType);
											arcsRST.add(arcRST);
											bFound = true;
											break;
