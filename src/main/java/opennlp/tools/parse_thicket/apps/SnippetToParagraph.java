...
public class SnippetToParagraph extends ContentGeneratorSupport /* RelatedSentenceFinder */ {
	...
	private static Logger LOG = Logger.getLogger("com.become.parse_thicket.apps.SnippetToParagraph");
	...
	public HitBase formTextFromOriginalPageGivenSnippetDirect(HitBase item) {
		...
		for (String fragment : allFragms) {
			...
			if (fragment.indexOf("_should_find_orig_") > -1 && sents != null && sents.size() > 0)
				try {
			...
			if (pageSentenceProc != null) {
				...
				f.fragment = fragment;
				result.add(f);
				LOG.info("Accepted sentence: " + pageSentenceProc + "| with title= " + title);
				LOG.info("For fragment = " + fragment);
			} else
				LOG.info("Rejected sentence due to wrong area at webpage: " + pageSentence);
		}
