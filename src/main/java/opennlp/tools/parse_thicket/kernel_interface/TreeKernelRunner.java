...
public class TreeKernelRunner {
	...
	public void runLearner(String dir, String learning_file, String model_file) {
		if (!dir.endsWith("/"))
			dir += "/";
		String[] runString = new String[] { dir + "svm_learn", "-t", "5", "-j", "2", "-W", "A", dir + learning_file,
				dir + model_file };
	...
	public void runLearnerWin(String dir, String learning_file, String model_file) {
		...
		if (!dir.endsWith("\\"))
			dir += "\\";
		String[] runString = new String[] { dir + "svm_learn.exe", "-t", "5", "-j", "2", "-W", "A", dir + learning_file,
				dir + model_file };
		runEXE(runString, dir);
	}
	...
	public void runClassifier(String dir, String example_file, String model_file, String predictions_file) {
		if (!dir.endsWith("/"))
			dir += "/";
		String[] runString = new String[] { dir + "svm_classify", dir + example_file, dir + model_file,
				dir + predictions_file };
		runEXE(runString, dir);
	}
	...
	public void runClassifierWin(String dir, String example_file, String model_file, String predictions_file) {
		...
		if (!dir.endsWith("\\"))
			dir += "\\";
		String[] runString = new String[] { dir + "svm_classify.exe", dir + example_file, dir + model_file,
				dir + predictions_file };
		runEXE(runString, dir);
	}
 ...
 * settings in format <i>name</i>=<i>value</i>.
 * 
 * @param dir the working directory of the subprocess, or <tt>null</tt> if the
 * subprocess should inherit the working directory of the current process.
 * 
 * Ð’ Ð´Ð¸Ñ�Ñ‚Ñ€Ð¸Ð±ÑƒÑ‚Ð¸Ð²Ðµ Ð´Ð²Ð° exe-Ñ„Ð°Ð¹Ð»Ð°: svm_learn.exe Ð¸
 * svm_classify.exe.
 * 
 * 1. svm_learn.exe Ð±ÐµÑ€ÐµÑ‚ Ñ„Ð°Ð¹Ð» Ñ� Ð¿Ñ€Ð¸Ð¼ÐµÑ€Ð°Ð¼Ð¸,
 * Ð¾Ð±Ñ€Ð°Ð±Ð°Ñ‚Ñ‹Ð²Ð°ÐµÑ‚ ÐµÐ³Ð¾, Ñ�Ñ‚Ñ€Ð¾Ð¸Ñ‚ Ñ„Ð°Ð¹Ð» model Ð¼
 * Ð¿Ñ€Ð°Ð²Ð¸Ð»Ð°Ð¼Ð¸ Ð¾Ð±ÑƒÑ‡ÐµÐ½Ð¸Ðµ.
 * 
 * ÐŸÑ€Ð¸Ð¼ÐµÑ€Ñ‹ Ð·Ð°Ð¿ÑƒÑ�ÐºÐ°: svm_learn -t 5 learning_file model_file -
 * Ñ�Ñ‚Ð¾ Ñ�Ð°Ð¼Ñ‹Ð¹ Ð¿Ñ€Ð¾Ñ�Ñ‚Ð¾Ð¹ Ð²Ð°Ñ€Ð¸Ð°Ð½Ñ‚ Ð·Ð°Ð¿ÑƒÑ�ÐºÐ°,
 * SubSetTreeKernel (Ð´Ð¾Ð¿ÑƒÑ�ÐºÐ°ÑŽÑ‚Ñ�Ñ� Ñ€Ð°Ð·Ñ€Ñ‹Ð²Ñ‹ Ð¿Ñ€Ð¸ Ð¾Ð±Ñ…Ð¾Ð´Ðµ
 * Ð´ÐµÑ€ÐµÐ²ÑŒÐµÐ²)
 * 
 * svm_learn -t 5 -D 0 learning_file model_file - Ð´Ñ€ÑƒÐ³Ð¾Ð¹ Ð²Ð°Ñ€Ð¸Ð°Ð½Ñ‚
 * Ñ�Ð´Ñ€Ð°, SubTreeKernel
 * 
 * ÐŸÑ€Ð¸Ð¼ÐµÑ€ Ñ„Ð°Ð¹Ð»Ð° Ð»ÐµÐ¶Ð¸Ñ‚ Ð½Ð° ÐµÐ³Ð¾ Ñ�Ñ‚Ñ€Ð°Ð½Ð¸Ñ‡ÐºÐµ. Ð¢Ð°Ð¼
 * Ð¶Ðµ Ð¾Ð¿Ð¸Ñ�Ð°Ð½Ð¸Ðµ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ð¾Ð².
 * 
 ...
 * Ñ�Ñ‚Ñ€Ð°Ð½Ð¸Ñ‡ÐºÐµ ÐœÐ¾Ñ�ÐºÐ¸Ñ‚Ñ‚Ð¸. ÐœÐ¾Ð¶Ð½Ð¾ Ñ�Ñ€Ð°Ð·Ñƒ Ð¶Ðµ
 * ÑƒÐºÐ°Ð·Ñ‹Ð²Ð°Ñ‚ÑŒ, Ðº ÐºÐ°ÐºÐ¾Ð¼Ñƒ ÐºÐ»Ð°Ñ�Ñ�Ñƒ Ð¾Ñ‚Ð½Ð¾Ñ�Ð¸Ñ‚Ñ�Ñ�
 * Ð¿Ñ€Ð¸Ð¼ÐµÑ€ (1 Ð¸Ð»Ð¸ -1 Ð² Ð½Ð°Ñ‡Ð°Ð»Ðµ Ñ�Ñ‚Ñ€Ð¾ÐºÐ¸). Ð’ Ñ�Ñ‚Ð¾Ð¼
 * Ñ�Ð»ÑƒÑ‡Ð°Ðµ Ñ‚Ð¾Ñ‡Ð½Ð¾Ñ�Ñ‚ÑŒ Ð¸ Ð¿Ð¾Ð»Ð½Ð¾Ñ‚Ð° Ð¾Ñ†ÐµÐ½Ð¸Ð²Ð°ÑŽÑ‚Ñ�Ñ�
