...
public class PT2ThicketPhraseBuilder {
	...
	private static Logger log = Logger.getLogger("opennlp.tools.parse_thicket.matching.PT2ThicketPhraseBuilder");

	/*
	 * Building phrases takes a Parse Thicket and forms phrases for each
	 * sentence individually Then based on built phrases and obtained arcs, it
	 * builds arcs for RST Finally, based on all formed arcs, it extends phrases
	 * with thicket phrases
	 */
	...
	public List<List<ParseTreeNode>> buildPT2ptPhrases(ParseThicket pt) {
		...
		List<WordWordInterSentenceRelationArc> arcsRST = rstBuilder.buildRSTArcsFromMarkersAndCorefs(pt.getArcs(),
				sentNumPhrases, pt);
		...
		List<List<ParseTreeNode>> expandedPhrases = expandTowardsThicketPhrases(phrasesAllSent, pt.getArcs(),
				sentNumPhrases, pt);
		return expandedPhrases;
	}

	/*
	 * Take all phrases, all arcs and merge phrases into Thicket phrases. Then
	 * add the set of generalized (Thicket) phrases to the input set of phrases
	 * phrasesAllSent - list of lists of phrases for each sentence sentNumPhrase
	 * - map , gives for each sentence id, the above list arcs - arcs formed so
	 * far pt - the built Parse Thicket
	 */
	protected List<List<ParseTreeNode>> expandTowardsThicketPhrases(List<List<ParseTreeNode>> phrasesAllSent,
			List<WordWordInterSentenceRelationArc> arcs, Map<Integer, List<List<ParseTreeNode>>> sentNumPhrases,
			ParseThicket pt) {
		...
		for (int nSent = 0; nSent < pt.getSentences().size(); nSent++) {
			for (int mSent = nSent + 1; mSent < pt.getSentences().size(); mSent++) {
				// for given arc, find phrases connected by this arc and add to
				// the list of phrases
				for (WordWordInterSentenceRelationArc arc : arcs) {
					...
					if (nSent == fromIndex && mSent == toIndex) {
						...
						// for the given arc arc, find phrases which are
						// connected by it
	...
	/*
	 * check that one phrase is subphrase of another by lemma (ignoring other
	 * node properties) returns true if not found different word
	 */

	private boolean identicalSubPhrase(List<ParseTreeNode> lFromFound, List<ParseTreeNode> lToFound) {
	...
	private List<ParseTreeNode> append(List<ParseTreeNode> lFromFound, List<ParseTreeNode> lToFound) {
	 ...
	 * <10>VP'its':PRP$, <11>VP'dispute':NN, <12>VP'over':IN, <13>VP'its':PRP$,
	 * <14>VP'work':NN, <15>VP'on':IN, <16>VP'nuclear':JJ, <17>VP'weapons':NNS],
	 * [<4>VP'accept':VB, <5>VP'the':DT, <6>VP'UN':NNP, <7>VP'proposal':NN,
	 * <8>VP'to':TO, <9>VP'end':VB, <10>VP'its':PRP$, <11>VP'dispute':NN,
	 * <12>VP'over':IN, <13>VP'its':PRP$, <14>VP'work':NN, <15>VP'on':IN,
	 * <16>VP'nuclear':JJ, <17>VP'weapons':NNS], [<5>NP'the':DT, <6>NP'UN':NNP,
	 * <7>NP'proposal':NN], [<8>VP'to':TO, <9>VP'end':VB, <10>VP'its':PRP$,
	 * <11>VP'dispute':NN, <12>VP'over':IN, <13>VP'its':PRP$, <14>VP'work':NN,
	 * <15>VP'on':IN, <16>VP'nuclear':JJ, <17>VP'weapons':NNS], [<9>VP'end':VB,
	 * <10>VP'its':PRP$, <11>VP'dispute':NN, <12>VP'over':IN, <13>VP'its':PRP$,
	 * <14>VP'work':NN, <15>VP'on':IN, <16>VP'nuclear':JJ, <17>VP'weapons':NNS],
	 * [<10>NP'its':PRP$, <11>NP'dispute':NN], [<12>PP'over':IN,
	 * <13>PP'its':PRP$, <14>PP'work':NN, <15>PP'on':IN, <16>PP'nuclear':JJ,
	 * <17>PP'weapons':NNS], [<13>NP'its':PRP$, <14>NP'work':NN, <15>NP'on':IN,
	 * <16>NP'nuclear':JJ, <17>NP'weapons':NNS], [<13>NP'its':PRP$,
	 * <14>NP'work':NN], [<15>PP'on':IN, <16>PP'nuclear':JJ,
	 * <17>PP'weapons':NNS], [<16>NP'nuclear':JJ, <17>NP'weapons':NNS]]
	 * 
	 * 
	 */
	...
	private List<ParseTreeNode> assignIndexToNodes(List<ParseTreeNode> node, List<ParseTreeNode> sentence) {
	...
	/*
	 * [[NP'':], ['(NNP':Iran)], [VP'':], ['(VBZ':refuses)], [VP'':],
	 * ['(TO':to)], [VP'':], ['(VB':accept)], [NP'':], ['(DT':the)],
	 * ['(NNP':UN)], ['(NN':proposal)], [VP'':], ['(TO':to)], [VP'':],
	 * ['(VB':end)], [NP'':], ['(PRP$':its)], ['(NN':dispute)], [PP'':],
	 * ['(IN':over)], [NP'':], [NP'':], ['(PRP$':its)], ['(NN':work)], [PP'':],
	 * ['(IN':on)], [NP'':], ['(JJ':nuclear)], ['(NNS':weapons)], ['(.':.)]]
	 * 
	 * [[NP'':], ['(NNP':Iran)], [VP'':], ['(VBZ':refuses)], [VP'':],
	 * ['(TO':to)], [VP'':], ['(VB':accept)], [NP'':], ['(DT':the)],
	 * ['(NNP':UN)], ['(NN':proposal)], [VP'':], ['(TO':to)], [VP'':],
	...
	protected List<ParseTreeNode> parsePhrase(String value) {
		...
		} else if (pos_value != null && pos_value.length == 2) {
	...
	public List<ParseTreeNode> parsePhrase(String value, String fullDump) {
		...
		String flattened = fullDump.replace("(ROOT", "").replace("(NP", "").replace("(VP", "").replace("(PP", "")
				.replace("(ADVP", "").replace("(UCP", "").replace("(ADJP", "").replace("(SBAR", "").replace("(PRT", "")
				.replace("(WHNP", "").replace("))))", ")").replace(")))", ")").replace("))", ")").replace("   ", " ")
	...
	public static void main(String[] args) {
		...
		String para = "Last Wednesday, world powers reached agreement with Iran on limiting Iranian nuclear activity in return for the lifting of sanctions. "
		/*
		 * +
...
/*
 * The Ukrainian government, Western leaders and Nato all say there is clear
 * evidence that Russia is helping the rebels in the eastern Donetsk and Luhansk
 * regions with heavy weapons and soldiers. Independent experts echo that
 * accusation. Moscow denies it, insisting that any Russians serving with the
 * rebels are volunteers.
 * 
 */
