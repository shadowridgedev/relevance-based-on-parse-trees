...
public class LemmaFormManager {
	...
	public String matchLemmas(PStemmer ps, String lemma1, String lemma2, String POS) {
		if (POS == null) {
			return null;
		}
		...
		if (lemma1.endsWith("_xyz") || lemma2.endsWith("_xyz")) {
			...
			if (lemma1.equals(lemma2)) {
				return lemma1;
			} else { // trying to check if nouns and different plural/single
						// form
				if (POS.equals("NN") || POS.equals("NP")) {
					if ((lemma1.equals(lemma2 + "s") || lemma2.equals(lemma1 + "s")) || lemma1.endsWith(lemma2)
							|| lemma2.endsWith(lemma1) || lemma1.startsWith(lemma2) || lemma2.startsWith(lemma1))
						return lemma1;
				}
				return "fail";
			}
	 ...
	 * ParseTreePath chunkToAdd) { List<String> lemmasWithMustOccur =
	 * ch1.getLemmas(); lemmasWithMustOccur.addAll(ch2.getLemmas());
	 * List<String> res = chunkToAdd.getLemmas(); for (String lem :
	 * lemmasWithMustOccur) { if (lem.endsWith("_xyz")) { String pureLem =
	 * lem.replace("_xyz", ""); if (!res.contains(pureLem)) { // should occur
	 * but does not return false; }// failed the test } } return true; }
	 */
}
