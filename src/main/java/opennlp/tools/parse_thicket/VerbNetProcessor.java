...
public class VerbNetProcessor implements IGeneralizer<Map<String, List<String>>> {
	...
	static private String pathToVerbnet = null; // new File( "."
	...
	// abandon (leave-51.2 leave-51.2 ) (NP V NP.source ) (Transitivebasically,
	// locative preposition drop of "from" ) (
	public StringBuilder buildTreeRepresentationForTreeKernelLearning(String verb) {
		...
		sb.append(") (");
		for (int i = 0; i < frames.size(); i++) {
			sb.append(frames.get(i).getSecondaryType().getID().replace(".", "").replace(",", " ").replace("\"", "-")
	...
	public void testIndex() throws Exception {
		...
		while (iter.hasNext()) {
			...
			for (IFrame f : frames) {
				try {
					System.out.println(f.getPrimaryType().getID() + " => " + f.getXTag() + " >> "
							+ f.getSecondaryType().getID() + " : " + f.getExamples().get(0));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public static void main(String[] args) {
		...
		/*
		 * try { proc.testIndex(); } catch (Exception e) { // TODO
