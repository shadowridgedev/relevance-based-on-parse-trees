...
public class LevensteinDistanceFinder {
	...
	public static double matchLevensteinDistance(String str1, String str2) {
		...
		return levensteinDistance(str1, str2, 1, 10, 1, 10) / (str1.length() + 1 + str2.length());
	}
	 ...
	 */
	public static double levensteinDistance(String str1, String str2, int letterInsDelCost, int digitInsDelCost,
			int letterReplaceCost, int digitReplaceCost) {
		...
		for (int j = 1; j < length2; j++) {
			int cost = letterInsDelCost; // 1 is a cost for inserting a
											// character
			...
			int min = 0;
			for (int i = 1; i < length1; i++) {
				cost = letterInsDelCost; // 1 is a cost for inserting a
											// character
				if (Character.isDigit(str1.charAt(i - 1))) {
					cost = digitInsDelCost;
				}
				...
				if (str1.charAt(i - 1) != str2.charAt(j - 1)) {
					...
					if (Character.isDigit(str1.charAt(i - 1)) || Character.isDigit(str2.charAt(j - 1))) {
	...
	public static double distanceBetweenStringArraysAsSpaceSepar(String line1, String line2) {
