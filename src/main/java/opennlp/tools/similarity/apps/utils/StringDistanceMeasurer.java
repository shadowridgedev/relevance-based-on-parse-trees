...
public class StringDistanceMeasurer {
	...
	public double measureStringDistance(String str1, String str2) {
		...
		try {
			...
			if (str1.equals(str2)) // || str1.endsWith(str2) ||
									// str2.endsWith(str1))
									// bg 03-2011
				return 1.0;
			...
			for (String w : strListOverlap) {
				if (w.toLowerCase().equals(w)) // no special interest word
					...
					lOverlap += 2; // if capitalized, or specific word =>
									// important so
									// double score
			}
			result = Math.pow((double) (lOverlap * lOverlap) / (double) l1 / (double) l2, 0.4);
			...
			for (String w1 : str1List) {
				for (String w2 : str2List) {
					if (w1.length() > MIN_STRING_LENGTH_FOR_DISTORTED_WORD || !w1.toLowerCase().equals(w1))
						if (w2.length() > MIN_STRING_LENGTH_FOR_DISTORTED_WORD || !w2.toLowerCase().equals(w2))
			...
			// double ld = LevensteinDistanceFinder. levensteinDistance(str1,
			// str2, 1,
			// 10, 1, 10);
		...
		/*
		 * if (result>MIN_SCORE_FOR_LING) { List<List<ParseTreeChunk>>
		 * matchResult = pos.matchOrigSentencesCache(str1, str2);
		 * linguisticScore =
	...
	public static void main(String[] args) {
		...
		// common sub-chunk = [VBD-started IN-in NNS-50s IN-with PRP$-our
		// NN-help ]
