...
public class GeneratedSentenceProcessor {
	...
	public static String[] occurs = new String[] { "click here", "wikipedia", "retrieved", "isbn", "http", "www.",
			"copyright", "advertise", "(accessed", "[edit]", "[citation needed]", "site map", "email updates",
			"contact us", "rss feeds", "cite this site", "operating hours", "last modified", "product catalog",
			"days per week", "leave a comment", "corporate information", "employment opportunities", "terms of use",
			"private policy", "parental guidelines", "copyright policy", "ad choices", "about us", "about our ads",
			"privacy policy", "terms of use", "click for", "photos", "find the latest", "terms of service",
			"clicking here", "skip to", "sidebar", "Tags:", "available online", "get online", "buy online", "not valid",
			"get discount", "official site", "this video",
			// "this book",
			"this product", "paperback", "hardcover", "audio cd", "related searches", "permission is granted", "[edit",
			"edit categories", "free license", "permission is granted", "under the terms", "rights reserved",
			"wikipedia", "recipient of", "this message", "mailing list", "purchase order", "mon-fri", "email us",
			"privacy pol", "back to top", "click here", "for details", "assistance?", "chat live", "free shipping",
			"company info", "satisfaction g", "contact us", "menu.", "search.", "sign in", "home.", "additional terms",
			"may apply" };

	public static String[] occursStartsWith = new String[] { "fax", "write", "email", "contact", "conditions",
			"chat live", "we ", "the recipient", "day return", "days return", "refund it", "your money",
			"purchase orders", "exchange it ", "return it", "day return", "days return", "subscribe", "posted by",
			"below", "corporate", "this book" };
	...
	public static String acceptableMinedSentence(String sent) {
		...
		String[] brakets = StringUtils.split(sent.replace('(', '#').replace(')',
				'#')/* .replace('[', '#').replace(']', '#') */, '#');
		if ((float) periods.length / (float) spaces.length > 0.2) {
		...
		if (StringUtils.split(sent, '|').length > 2 || StringUtils.split(sent, '>').length > 2) {
			// System.out.println("Rejection: too many |s or >s in sent
			// ='"+sent);
			return null;
		}
		...
		if ((sentWrongSym.length() - sentTry.length()) >= 4 && sentTry.length() < 200) // twice
																						// ot
																						// more
			return null;
		...
		sent = sent.replace('[', ' ').replace(']', ' ').replace("_should_find_orig_", "").replace(".   .", ". ")
				...
				/*
				 * .replace("2009", "2011") .replace("2008",
				 * "2011").replace("2006", "2011") .replace("2007", "2011").
				 */ replace("VIDEO:", " ").replace("Video:", " ").replace("no comments", " ").replace("  ", " ").replace("  ", " ").replace("(more.)", "").replace("more.", "").replace("<more>", "").replace("[more]", "").replace(".,", ".").replace("&lt;", "").replace("p&gt;", "").replace("product description", "");

		// sent = sent.replace("Click here. ","").replace("Share
		// this:.","").replace("Facebook.","").
		// replace("Twitter." Email. Google. Print. Tumblr. Pinterest. More.
		// Digg. LinkedIn. StumbleUpon. Reddit. Like this: Like Loading.. ")
	...
	public static boolean isProhibitiveWordsOccurOrStartWith(String sentenceLowercase) {
		...
		for (String o : occursStartsWith) {
			if (sentenceLowercase.startsWith(o)) {
				// System.out.println("Found prohibited occurrence Start With "+
	...
	public static String normalizeForSentenceSplitting(String pageContent) {
		pageContent.replace("Jan.", "January").replace("Feb.", "February").replace("Mar.", "March")
				.replace("Apr.", "April").replace("Jun.", "June").replace("Jul.", "July").replace("Aug.", "August")
				.replace("Sep.", "September").replace("Oct.", "October").replace("Nov.", "November")
 ...
 * sentTry.indexOf("copyright") > -1 || sentTry.indexOf("operating hours") > -1
 * || sentTry.indexOf("days per week") > -1 || sentTry.indexOf("click for") > -1
 * || sentTry.indexOf("photos") > -1 || sentTry.indexOf("find the latest") > -1
 * || sentTry.startsWith("subscribe") || sentTry.indexOf("Terms of Service") >
 * -1 || sentTry.indexOf("clicking here") > -1 || sentTry.indexOf("skip to") >
 * -1 || sentTry.indexOf("sidebar") > -1 || sentTry.indexOf("Tags:") > -1 ||
 * sentTry.startsWith("Posted by") || sentTry.indexOf("available online") > -1
 * || sentTry.indexOf("get online") > -1 || sentTry.indexOf("buy online") > -1
 * || sentTry.indexOf("not valid") > -1 || sentTry.indexOf("discount") > -1 ||
 * sentTry.indexOf("official site") > -1 || sentTry.indexOf("this video") > -1
 * || sentTry.indexOf("this book") > -1 || sentTry.indexOf("this product") > -1
 * || sentTry.indexOf("paperback") > -1 || sentTry.indexOf("hardcover") > -1 ||
 * sentTry.indexOf("audio cd") > -1 || sentTry.indexOf("related searches") > -1
 * || sentTry.indexOf("permission is granted") > -1 || sentTry.indexOf("[edit")
 * > -1 || sentTry.indexOf("edit categories") > -1 ||
 * sentTry.indexOf("free license") > -1 ||
 * sentTry.indexOf("permission is granted") > -1 ||
 * sentTry.indexOf("under the terms") > -1 || sentTry.indexOf("rights reserved")
 * > -1 || sentTry.indexOf("wikipedia") > -1 || sentTry.endsWith("the") ||
 * sentTry.endsWith("the.") || sentTry.startsWith("below") ||
 ...
 * ||sentTry.indexOf( "chat live")>-1 ||sentTry.startsWith( "we ")
 * ||sentTry.indexOf( "the recipient")>-1 ||sentTry.indexOf( "day return")>-1
 * ||sentTry.indexOf( "days return")>-1
 * 
 * ||sentTry.startsWith( "fax") ||sentTry.indexOf( "refund it")>-1 ||
 * sentTry.indexOf( "your money")>-1 ||sentTry.indexOf( "cond???")>-1
 * ||sentTry.indexOf( "purchase orders")>-1 ||sentTry.startsWith(
 * "exchange it ") ||sentTry.indexOf( "return it")>-1 ||sentTry.indexOf(
 * "day return")>-1 ||sentTry.indexOf( "days return")>-1 ) return null;
 * 
 */