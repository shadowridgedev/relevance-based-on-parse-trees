...
public class SnippetToParagraphAndSectionHeaderContent {
	...
	private static Logger LOG = Logger.getLogger("opennlp.tools.chatbot.SnippetToParagraphAndSectionHeaderContent");
	...
	private Pair<String[], Map<String, String>> extractSentencesAndSectionMapFromPage(String url) {
		...
		for (String h : acceptedSectionHeaders) {
			try {
				...
				if (pos < 0)
					continue;
				String areaContent = downloadedPage.substring(pos + h.length(),
						Math.min(pos + h.length() + 500, downloadedPage.length())) + "._.";
				if (areaContent != null)
