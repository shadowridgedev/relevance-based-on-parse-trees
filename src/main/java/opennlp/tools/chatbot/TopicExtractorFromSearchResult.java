...
public class TopicExtractorFromSearchResult {
	...
	private static Logger LOG = Logger.getLogger("opennlp.tools.chatbot.TopicExtractorFromSearchResult");
	...
	public EntityExtractionResult extractEntities(String para) {
		...
		List<String> extractedNerPhrasesStr = new ArrayList<String>(), extractedNerExactStr = new ArrayList<String>(),
				extractedSentimentPhrasesStr = new ArrayList<String>(),
				extractedNONSentimentPhrasesStr = new ArrayList<String>(),
				extractedNerPhraseTags = new ArrayList<String>();
		...
		for (List<ParseTreeNode> phrase : phrases) {
			...
			for (ParseTreeNode word : phrase) {
				...
				if (word.getPos().startsWith(
						"CD") /* || word.getPos().indexOf("PRP")>-1 */ )
					bAccept = false;
			}
			if (!bAccept)
				continue;
	...
	private boolean isCoveredByQueryPhrase(List<ParseTreeNode> phrase, List<String> queryKeywords) {
		...
		for (ParseTreeNode p : phrase) {
			if (!queryKeywords.contains(p.getWord().toLowerCase())) {
				 ...
				 * else { System.out.println("found match: "+ queryKeywords +
				 * " => " + p.getWord().toLowerCase()); }
				 */
		}
...
/*
 * Naïve sentiment prediction systems work just by looking at words in
 ...
 * meaning of longer phrases. However, in most applications just taking
 * individual sentences into account do not give accurate results and rhetoric
 * information needs to be taken into account to determine the overall sentiment
 * of a paragraph and then back to the individual sentence level.
 */
