...
public class ClarificationExpressionGenerator {
	private static Logger LOG = Logger.getLogger("opennlp.tools.chatbot.ClarificationExpressionGenerator");
	// session data
	...
	public String generateClarification(String query, List<ChatIterationResult> searchRes0) {
		...
		if (searchRes0.get(0).getEeResult() == null) // not real search results,
														// then expect
														// clarification in
														// title
		{
			...
			// TODO: we assume no noise keywords in the query such as "tell me
			// about ..."
			currentEntity = query;
			...
			try {
				...
				for (String c : candidates) {
					if (c.length() > maxLen
							...
							&& acceptableClarifPhrase(c) && meas.measureStringDistance(c, query) < 0.7) {
						bestPhrase = c;
						maxLen = c.length();
					}
				}
		...
		if (this.originalQuestion == null) {// very first session
			for (ChatIterationResult extrPhrases : searchRes0) {
				...
				try {
					if (extrPhrases.selectedClarificationPhrase != null
							&& extrPhrases.selectedClarificationPhrase.length() > 2) {
					...
					if (extrPhrases.firstClarificationPhrase != null
							&& extrPhrases.firstClarificationPhrase.length() > 2) {
						int randomIndex = new Float(Math.random() * (float) clarifPrefixes.length).intValue();
						clarificationStr += clarifPrefixes[randomIndex] + extrPhrases.firstClarificationPhrase + "'["
								+ count + "]. ";
						if (count % 3 == 0)
							clarificationStr += "\n";
					}
	...
	/*
	 * private List<String> selectBestPhrases(List<String> candidates){ // now
	 * find the longest and the first one String bestPhrase = ""; int maxLen =
	 * -1; int bestIndex = -1; if (candidates.get(0).length()>5)
	 * extrPhrases.firstClarificationPhrase=candidates.get(0);
	 * candidates.remove(extrPhrases.firstClarificationPhrase);
	 * 
	 * for(String c: candidates){ try { if (c.length()>maxLen &&
	 * meas.measureStringDistance(c, extrPhrases.firstClarificationPhrase)<0.8){
	 * bestPhrase = c; maxLen = c.length(); } } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */

	/*
	 * , How to pay my credit card bill with another credit card ... => [] [my
	 * credit card bill] , Can You Use One Credit Card to Pay Another Credit
	 * Card Bill? => [] [] , Can You Pay a Credit Card With a Credit Card? |
	 * Credit.com => [] [any of these scenarios, these scenarios, apply to you,
	 * your credit cards, to pay another credit card bill] , Can I pay credit
	 * card with another credit card? YEP! => [] [way, can pay my credit card
	 * bill, pay my credit card bill, my credit card bill, wants to get their
	 * credit, to get their credit, get their credit] , How to Pay a Credit Card
	 * Bill With Another Credit Card ... => [] [to pay off another, balance
	 * transfers, easiest way, is via a balance transfer, balance transfer, cash
	 * advances you, cash advances, taking a cash advance from another card,
	 * cash advance] , Should I use one credit card to pay off another? | Credit
	 * ... => [] [have a question, question, have advice to share, advice to
	 * share, to share, combined knowledge and experience, everyone in the
	 * credit karma community, credit karma community] , can i pay off my 1
	 * credit card with another? | Yahoo Answers => [] [small bill, same time,
	 * my capitalone card, min pay, received it, my capitalone card .] , Can You
	 * Pay a Credit Card With a Credit Card? | Fox Business => [] [any of these
	 * scenarios, these scenarios, apply to you, your credit cards, to pay
	...
	boolean acceptableClarifPhrase(String ph) {
		...
		if (ph.startsWith("our") || ph.startsWith("my") || ph.startsWith("no ") || ph.startsWith("could ")
				|| ph.startsWith("no ") || ph.startsWith("get ") || ph.startsWith("contact us")
				|| ph.indexOf("editorial ") > -1 || ph.startsWith("any ") || ph.indexOf("also like") > -1
				|| ph.indexOf("also like") > -1 || ph.indexOf("clickred") > -1 || ph.startsWith("join ")
				|| ph.indexOf("enjoy the article") > -1 || ph.indexOf("company info") > -1
	...
	public String searchProductPage(String query) {
		...
		String cleanedSent = bestS.replace("   ", " ").replace("  ", " ").replace("  ", " ").replace("  ", " ")
				.replace("  ", " ");
		return bestS;
	}
}
...
/*
 * It can be related to :'your card issuer 's payment guidelines'. It can be
 * related to :'miss payments altogether'. It can be related to :'payment grace
 * period'. It can be related to :'making a late credit card payment'. Or is it
 * about :'your credit card company'. Did you mean: 'will begin charging a late
 * payment fee'. Did you mean: 'all the ways you can waste money'. Did you mean:
 * 'to avoid credit card late'. Or is it about :'claim interest and debt
 * recovery costs if'. It can be related to :'can drop your credit'.
 * 
 */