...
public class SearchSessionManager {
	private static Logger LOG = Logger.getLogger("opennlp.tools.chatbot.SearchSessionManager");
		...
		while (true) {
			...
			try {
				...
				if (queryType == 0) {
					...
					String selectedAnswer = clarificationExpressionGenerator
							.matchUserResponseWithGeneratedOptions(query);
					...
					List<ChatIterationResult> searchRes0 = searcher
					...
					for (int i = 0; i < searchRes0.size(); i++) {
						if (!clarificationExpressionGenerator
								.isEqualToAlreadyGivenAnswer(searchRes0.get(i).getParagraph())) {
