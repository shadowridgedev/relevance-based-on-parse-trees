...
public class BingAPIProductSearchManager {
	...
	public List<HitBase> findProductByName(String name, int count) {
		List<HitBase> foundFBPages = search.runSearch("site:amazon.com" + " " + name + " reviews", 10);
		...
		for (HitBase h : foundFBPages) {
			...
			int overlapCount = 0;
			/*
			 * for(String commonWord:merchantWords){ if
			 * (title.indexOf(commonWord+" ")>-1 ||
			 * title.indexOf(" "+commonWord)>-1){ overlapCount++;
			 * System.out.println(" found word "+ commonWord +
			 * " in title = "+title); } } float coverage =
			 * (float)overlapCount/(float) (merchantWords.length); if
			 * ((coverage>0.4 || (coverage>0.5f && merchantWords.length <4 )))
			 */ results.add(h);
		}
		return results;
	}
