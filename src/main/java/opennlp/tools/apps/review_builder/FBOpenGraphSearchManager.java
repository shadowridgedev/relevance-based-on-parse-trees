...
public class FBOpenGraphSearchManager {
	...
	public FBOpenGraphSearchManager() {
		profiles = ProfileReaderWriter.readProfiles(
	...
	public List<Event> getFBEventsByName(String event) {
		...
		for (int i = 0; i < NUM_TRIES; i++) {
			try {
				Connection<Event> publicSearch = mFBClient.fetchConnection("search", Event.class,
	...
	public Long getFBPageLikes(String merchant) {
		...
		for (int i = 0; i < NUM_TRIES; i++) {
			try {
				Connection<Page> publicSearch = mFBClient.fetchConnection("search", Page.class,
