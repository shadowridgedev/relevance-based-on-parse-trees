...
public class WebPageReviewExtractor extends WebPageExtractor {
	...
	public String[] removeDuplicates(String[] hits) {
		...
		try {
			...
			if (hitsDedup.size() < hits.length) {
				System.out.println(
						"Removed duplicates from relevant search results, including " + hits[idsToRemove.get(0)]);
			}
		} catch (Exception e) {
	...
	public ReviewObj extractSentencesWithPotentialReviewPhrases(String url) {
		...
		if (productFeatures != null) {
			for (String item : productFeatures) {
				...
				if (item.length() > 80 && MinedSentenceProcessor.acceptableMinedSentence(item) == null) {
					System.out.println(
							"Rejected sentence by GeneratedSentenceProcessor.acceptableMinedSentence = " + item);
					continue;
				}
		...
		if (item == null) {// title="4.0 out of 5 stars" ><span>4.0 out of 5
							// stars</span>
	...
	private List<String> cleanProductFeatures(List<String> productFeaturesList) {
		...
		for (String feature : productFeaturesList) {
			if (feature.startsWith("Unlimited Free") || feature.startsWith("View Larger")
					|| feature.startsWith("View Larger") || feature.indexOf("shipping") > 0)
				continue;
			results.add(feature);
		}
		return results;
	...
	public List<String> formReviewsForAProduct(
			String name /* long bpid, String keywordsName */) {
		...
		try {
			...
			for (HitBase p : pagesForAProduct) {
				ReviewObj reviewObj = extractSentencesWithPotentialReviewPhrases(p.getUrl());
				// init with first element
				if (reviewObjTotal == null)
					reviewObjTotal = reviewObj;
				if (reviewObj == null)
					continue;
				String[] afterOriginalization = orig.convert(reviewObj.getOrigSentences(), p.getTitle(),
						reviewObj.getKeywordsName());
				...
				/*
				 * buf = reviewObjTotal.getOriginalizedSentences(); if
				 * (buf!=null && afterOriginalization!=null &&
				 * afterOriginalization.length>0){ List<String> b1 =
				 * Arrays.asList(afterOriginalization); List<String> b2 = new
				 * ArrayList<String>(); b2.addAll(buf); b2.addAll(new
				 * ArrayList<String>(b1));
			...
			/*
			 * String textReview = buildText(reviewObjTotal); try { if
			 * (textReview!=null && textReview.length()>60)
			 * ser.saveReviewsToDB(textReview, bpid,
			 * pagesForAProduct.get(0).getUrl(),
			 * pagesForAProduct.get(0).getTitle(),
			 * reviewObjTotal.getSentimentPhrases().toString(),
			 * reviewObjTotal.getRating()); } catch (Exception e) {
			...
			if (count % 2 == 0 && count < features.length)
				if (features[count] != null) {
					...
					if (!(features[count].endsWith("!") || features[count].endsWith("?")
							|| features[count].endsWith("?") || features[count].endsWith(".\"")))
						buf.append(". ");
				}
	...
	public static void main(String[] args) {
		...
		List<String> res = extractor.formReviewsForAProduct(// "McCulloch
															// 16-Inch 3.5 HP
															// Electric Chain
															// Saw");
				...
				// "denon Network Streaming A/V Home Theater receiver",
				// 209565926);
		...
		/*
		 * res= extractor. extractSentencesWithPotentialReviewPhrases(//
