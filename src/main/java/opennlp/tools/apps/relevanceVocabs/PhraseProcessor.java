...
public class PhraseProcessor {
	...
	public List<String> extractNounPhraseProductNameCandidate(String sentence) {
		...
		for (ParseTreeChunk ch : nPhrases) {
			 ...
			 * String[] qs = query.split(" "); boolean bAccept = true; for
			 * (String w : qs) { if (w.toLowerCase().equals(w)) // idf only two
			 * words then // has to be person name, // title or geo // location
			 * bAccept = false; } if (!bAccept) continue; }
			 */
		...
		/*
		 * if (queryArrayStr.size() < 1) { // release constraints on NP down to
		 * 2 // keywords for (ParseTreeChunk ch : nPhrases) { String query = "";
		 * int size = ch.getLemmas().size();
		 * 
		 * for (int i = 0; i < size; i++) { if
		 * (ch.getPOSs().get(i).startsWith("N") ||
		 * ch.getPOSs().get(i).startsWith("J")) { query += ch.getLemmas().get(i)
		 * + " "; } } query = query.trim(); int len = query.split(" ").length;
		 * if (len < 2) continue;
		 * 
		 * query = TextProcessor.fastTokenize(query.toLowerCase(), false)
		 * .toString().replace('[', ' ').replace(']', ' ').trim(); if
		 * (query.length() > 6) queryArrayStr.add(query); } } //queryArrayStr =
		 * Utils // .removeDuplicatesFromQueries(queryArrayStr); if (quoted1 !=
		 * null && ((quoted1.length() > 5 && !stopList.isCommonWord(quoted1)) ||
		 * quoted1 .length() > 10)) queryArrayStr.add(quoted1); if (quoted2 !=
		 * null && ((quoted2.length() > 5 && !stopList.isCommonWord(quoted2)) ||
		 * quoted2 .length() > 10)) queryArrayStr.add(quoted2);
		 */ return queryArrayStr;
	}

	public static void main(String[] args) {
		String sent = "Appliances and Kitchen Gadgets - CNET Blogs";
		// "The tablet phenomenon turns Silicon Valley upside down -
		// SiliconValley.com";
