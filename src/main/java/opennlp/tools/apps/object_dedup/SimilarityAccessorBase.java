...
public class SimilarityAccessorBase {
	...
	protected static final String[] englishPrepositions = new String[] { "a", "aboard", "about", "above", "absent",
			"across", "after", "against", "along", "alongside", "among", "around", "as", "at", "before", "behind",
			"below", "beneath", "between", "beyond", "but", "by", "despite", "down", "during", "except", "excluding",
			"failing", "following", "for", "from", "in", "including", "inside", "into", "like", "near", "next", "of",
			"off", "on", "onto", "only", "opposite", "out", "outside", "over", "pace", "past", "per", "since", "than",
			"through", "and", "thru", "till", "to", "toward", "under", "up", "upon", "versus", "with", "within", "you",
			"must", "know", "when" };
	...
	protected List<String> commonWordsInEventTitles = Arrays.asList(new String[] { "community", "party", "film",
			...
			"worship", "god", "training", "class", "development", "training", "class", "course", "our", "comedy",
			",fun", "musical", "group", "alliance", "session", "feeding", "introduction", "school", "conversation",
			"learning", "nursery", "unity", "trivia", "chat", "conference", "tuition", "technology", "teen",
			"communication", "reception", "management", "beginner", "beginning", "collabora", "reuninon", "political",
			"course", "age", "ages", "through", "grade", "networking", "workshop", "demonstration", "tuning", "program",
			"summit", "convention", "day", "night", "one", "two", "outfest", "three", "online", "writing", "seminar",
			"coach", ",expo", "advanced", "beginner", "intermediate", "earn", "free", "ii", "iii", "skills", "skill",
			"artist", "summer", "winter", "autumn", "spring", "camp", "vacation", "miscrosoft", "kid", "child", "kids",
			"children", "every", "everyone", "dancer", "dancers", "senior", "seniors", "basic", "elementary", "outfest",
			"2008", "2009", "2010", "2011", "2012", "monday", "tuesday", "wednesday", "thirsday", "friday", "saturday",
			"sunday", "mondays", "tuesdays", "wednesdays", "thirsdays", "fridays", "saturdays", "sundays", "men" // ?
	});
	...
	public List<String> getWordsThatShouldBeOnBothSidesEvents() {
		/*
		 * names.addAll(Arrays.asList(new String[] { "woman", "man", "women",
		 * "men", "womans", "mans", "womens", "mens", "boy", "girl", "boys",
		 * "girls", "men's", "women's", "woman's", "ice", // for disney
		 * "flight", "intermediate", "advanced", "beginner", // "tour", TODO
		 * special consideration "helicopter", "sexual", "junior", "jr" }));
		 */
		return null;
	...
	protected Boolean applySemanticNameSimilarityRule(Object es1, Object es2) {
		 ...
		 * es2.getVenueName().endsWith(es1.getVenueName()))) return false; if
		 * (Math.abs(es1.getStarttime().getTime() -
		 * es2.getStarttime().getTime()) > 100000) return false;
		 */
	...
	private Boolean attemptShortTitlesSimilarityInWebSpace(String name1, String name2) {
		...
		if (name1.startsWith(name2) || name1vv.startsWith(name2) || name1.startsWith(name2v)
				...
				|| name1vv.endsWith(name2) || name1.endsWith(name2v) || name1.endsWith(name2vv)
				|| name1.endsWith(name2vvv) || name1v.endsWith(name2v) || name1v.endsWith(name2vv)
				|| name2.endsWith(name1) || name2vv.endsWith(name1) || name2.endsWith(name1v)
				|| name1vvv.endsWith(name2vv) || name2.endsWith(name1vvv) || name2v.endsWith(name1v)
				|| name2v.endsWith(name1vv)) {
	...
	public Boolean applyBothSidesRuleEvent(String name1, String name2) {
		...
		if ((name1TokensC.size() > 0 && name1Tokens.size() < 3)
				|| (name1TokensC.size() > 1 && name1Tokens.size() < 5)) { // 'mens
																			// ==
																			// men;
																			// case
																			// !(name1TokensC.size()==2
	...
	public boolean verifyEventAttributesPost(List<String> name1Tokens, List<String> name2Tokens) {
		String[] attributeNamesPost = { "age", "ages", "game", "games", "grade", "grades", "level", "levels", "vs",
				"vs.", "versus", "pottery", "competition", "contest", "skill", "skills", "day", "only", "basic",
				"class", "completed",
				// "tour", ?
	...
	public boolean verifyEventAttributesPre(List<String> name1Tokens, List<String> name2Tokens) {
		...
		String[] attributeNamesPre = { "hour", "vs", "vs.", "versus", "pottery", "program", "day", "only",
				...
				"swing", "rumba", "samba", "doble", "violence", //
				// "level",
		...
		try {
			for (String attr : attributeNamesPre) {
				int agePos1 = name1Tokens.indexOf(attr);
				int agePos2 = name2Tokens.indexOf(attr);
				if (agePos1 > 0 && agePos2 > 0) { // not the first word is attr
													// name
					if (!name1Tokens.get(agePos1 - 1).equalsIgnoreCase(name2Tokens.get(agePos2 - 1)) && (agePos1 < 2
	...
	protected boolean bDifferentGroupOneSubnameOfAnother(String name1, String name2) {
		// first check a special case that both name1 and name2 are DIFFERENT
		// groups at last.fm
	...
	private boolean succeededMenWomenSportsRule(String name1, String name2) {
		...
		if (name1Tokens.contains("men") || name2Tokens.contains("men") || name1Tokens.contains("women")
				|| name2Tokens.contains("women") || name1Tokens.contains("disney") || name2Tokens.contains("disney")) { // all
																														// words
																														// should
																														// be
																														// the
	...
	private boolean succeededSpecialGroupsSymphoniesRule(String name1, String name2) {
		...
		if (name1Tokens.contains("orchestra") || name2Tokens.contains("symphony") || name2Tokens.contains("orchestra")
				...
				|| name1Tokens.contains("lang") || name2Tokens.contains("lang")) // special
																					// group
																					// 'lang
																					// lang'
	...
	private String normalizeGenderAndOtherAttributes(String name1) {
		...
		name1 = name1.replace(" man ", " men ").replace(" mans ", " men ").replace(" men's ", " men ")
				...
				.replace("gaea theatre festival", "tfestival"); // need regexp
																// for this
		return name1;
	}

	/*
	 * Main semantic similarity function which applies boundary cases rule and
	 * focus on web mining rule The main criteria for a commonality between
	 * titles: to form an entity, searchable on the web
	 */
	public DedupResult areNamesSemanticallyCloseWebMineCommonPart(String name1, String name2, String venue) {
		...
		if (commonPortion > 0.8 || name1Tokens.size() >= 4) { // after typical
																// title words
																// are revomed 4
																// looks OK
		...
		String entityExpression = name1Tokens.toString().replace('[', ' ').replace(']', ' ').replace(',', ' ')
				.replace("  ", " ").trim();
		/*
		 * // now try name merge reduced strings String entityExpression1 =
		 * name1TokensC.toString().replace('[', ' ').replace(']', '
		 * ').replace(',', ' ') .replace("  ", " ").trim(); String
		 * entityExpression2 = name2Tokens.toString().replace('[', '
		 * ').replace(']', ' ').replace(',', ' ') .replace("  ", " ").trim();
		 * 
		 * nameMergeScore = getAttemptedNameMerge(entityExpression1,
		 * entityExpression2); if (nameMergeScore>0){
		 * LOG.info("Found low NameMerge Distance for REDUCED name1 and name2");
		 * return new DedupResult("Found low  NameMerge Distance REDUCED", 2,
		 * true);
		 * 
		 * }
		 */
	...
	public DedupResult areNamesSemanticallyCloseInWebSearchSpace(String name1, String name2, Float thresh,
			boolean bStem) {
		...
		if (searchResult1 != null && searchResult2 != null) {
			for (HitBase item1 : searchResult1) {
				...
				for (HitBase item2 : searchResult2) {
					String lookup1 = item1.getTitle().replace("Facebook", "").replace("LinkedIn", "").replace("MySpace",
							"");
					String lookup2 = item2.getTitle().replace("Facebook", "").replace("LinkedIn", "").replace("MySpace",
							"");
					double d = 0;
					if (bStem)
					...
					if (d > thresh) // 0.8)
					{
						...
						reason.append("Found common search result title for group names '" + lookup1 + " < > " + lookup2
								+ " sim = " + d + "\n");
