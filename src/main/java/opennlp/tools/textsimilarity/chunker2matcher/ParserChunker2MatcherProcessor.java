...
public class ParserChunker2MatcherProcessor {
	 ...
	 * 
	 * @param para
	 *            input text string which is assumed to be a paragraph and is
	 *            split into sentences
	 * @return a list of lists of phrases with their POS tags for each phrase
	 *         type (noun, verb etc.)
	 */
	...
	public synchronized List<List<ParseTreeChunk>> formGroupedPhrasesFromChunksForSentence(String sentence) {
		 ...
		 * List<String> POSlist = node.getOrderedPOSList();
		 * 
		 * tags = POSlist.toArray(new String[0]); if (toks.length !=
		 * tags.length){
		 * LOG.info("disagreement between toks and tags; sent =  '"+sentence +
		 * "'\n tags = "+tags + "\n will now try this sentence in lower case" );
		 * node = parseSentenceNode(sentence.toLowerCase()); if (node==null){
		 * LOG.info("Problem parsing sentence '"+sentence); return null; }
		 * POSlist = node.getOrderedPOSList(); tags = POSlist.toArray(new
		 * String[0]); if (toks.length != tags.length){
		 * LOG.info("AGAIN: disagreement between toks and tags for lower case! "
		 * ); if (toks.length>tags.length){ String[] newToks = new
		 * String[tags.length]; for(int i = 0; i<tags.length; i++ ){ newToks[i]
		 * = toks[i]; } toks = newToks;
		 * 
		 * } else return null; } }
		 */
		...
		List<ParseTreeChunk> nounPhr = new ArrayList<ParseTreeChunk>(), prepPhr = new ArrayList<ParseTreeChunk>(),
				verbPhr = new ArrayList<ParseTreeChunk>(), adjPhr = new ArrayList<ParseTreeChunk>(),
	...
	public static List<List<SentenceNode>> textToSentenceNodes(List<List<Parse>> textParses) {
		...
		List<List<SentenceNode>> textNodes = new ArrayList<List<SentenceNode>>(textParses.size());
	...
	public static List<SentenceNode> paragraphToSentenceNodes(List<Parse> paragraphParses) {
		...
		List<SentenceNode> paragraphNodes = new ArrayList<SentenceNode>(paragraphParses.size());
	...
	public synchronized SentenceNode parseSentenceNode(String sentence, boolean normalizeText) {
	...
	protected void initializeTokenizer() {
		...
		try {
			...
			if (is != null) {
				try {
					is.close();
				} catch (IOException e) { // we swallow exception to support the
											// cached run
				}
			}
		}
	...
	private static SyntacticTreeNode toSyntacticTreeNode(Parse parse) {
		...
		if (type.equals(AbstractBottomUpParser.TOP_NODE) && childrenNodeList != null && childrenNodeList.size() > 0) {
			PhraseNode rootNode;
			try {
		...
		if (childrenNodeList != null && childrenNodeList.size() > 0) {
			// System.out.println("Found "+ type + " phrase = "+
			// childrenNodeList);
	...
	protected List<LemmaPair> listListParseTreeChunk2ListLemmaPairs(List<List<ParseTreeChunk>> sent1GrpLst) {
		List<LemmaPair> results = new ArrayList<LemmaPair>();
		if (sent1GrpLst == null || sent1GrpLst.size() < 1)
			return results;
		List<ParseTreeChunk> wholeSentence = sent1GrpLst.get(sent1GrpLst.size() - 1); // whole
																						// sentence
																						// is
																						// last
																						// list
 ...
 * opennlp.tools.textsimilarity.chunker2matcher.WordNode cannot be cast to
 * opennlp.tools.textsimilarity.chunker2matcher.PhraseNode at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * toSyntacticTreeNode(ParserChunker2MatcherProcessor.java:699) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * sentenceToSentenceNode(ParserChunker2MatcherProcessor.java:525) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * parseSentenceNode(ParserChunker2MatcherProcessor.java:554) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * parseSentenceNode(ParserChunker2MatcherProcessor.java:548) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * parseChunkSentence(ParserChunker2MatcherProcessor.java:282) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * formGroupedPhrasesFromChunksForSentence(ParserChunker2MatcherProcessor.java:
 * 355) at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * formGroupedPhrasesFromChunksForPara(ParserChunker2MatcherProcessor.java:250)
 * at
 * opennlp.tools.textsimilarity.chunker2matcher.ParserChunker2MatcherProcessor.
 * assessRelevance(ParserChunker2MatcherProcessor.java:747) at
 * opennlp.tools.similarity.apps.RelatedSentenceFinder.
 * augmentWithMinedSentencesAndVerifyRelevance(RelatedSentenceFinder.java:458)
 * at opennlp.tools.similarity.apps.RelatedSentenceFinder.generateContentAbout(
 * RelatedSentenceFinder.java:156) at
 */
