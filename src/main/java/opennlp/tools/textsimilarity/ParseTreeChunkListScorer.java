		...
		for (String l : chunk.getLemmas()) {
			...
			if (l.equals("*")) {
				if (pos.startsWith("CD")) { // number vs number gives high score
					...
					score += 0.7;
				} else if (pos.endsWith("_high")) { // if query modification
													// adds 'high'
					score += 1.0;
				} else {
					score += 0.1;
				...
				if (pos.startsWith("NN") || pos.startsWith("NP") || pos.startsWith("CD") || pos.startsWith("RB")) {
					score += 1.0;
				} else if (pos.startsWith("VB") || pos.startsWith("JJ")) {
					if (l.equals("get")) { // 'common' verbs are not that
											// important
						score += 0.3;
					} else {
						score += 0.5;
