...
public class ParseTreeMatcher {
	...
	public ParseTreeChunk generalizeTwoGroupedPhrasesOLD(ParseTreeChunk chunk1, ParseTreeChunk chunk2) {
	...
	public ParseTreeChunk prepositionalNNSTransform(ParseTreeChunk ch) {
		...
		if (!(Word_IN.equals("to") || Word_IN.equals("on") || Word_IN.equals("in") || Word_IN.equals("of")
				|| Word_IN.equals("with") || Word_IN.equals("by") || Word_IN.equals("from")))
			return ch;

		List<String> toShiftAfterPartPOS = ch.getPOSs().subList(indexIN + 1, ch.getPOSs().size());
		List<String> toShiftAfterPartLemmas = ch.getLemmas().subList(indexIN + 1, ch.getLemmas().size());
	...
	public ParseTreeChunk generalizeTwoGroupedPhrasesRandomSelectHighestScoreWithTransforms(ParseTreeChunk chunk1,
			ParseTreeChunk chunk2) {
		ParseTreeChunk chRes1 = generalizeTwoGroupedPhrasesRandomSelectHighestScore(chunk1, chunk2);
		ParseTreeChunk chRes2 = generalizeTwoGroupedPhrasesRandomSelectHighestScore(prepositionalNNSTransform(chunk1),
				chunk2);
		ParseTreeChunk chRes3 = generalizeTwoGroupedPhrasesRandomSelectHighestScore(prepositionalNNSTransform(chunk2),
				chunk1);

		ParseTreeChunk chRes = null;
		if (parseTreeChunkListScorer.getScore(chRes1) > parseTreeChunkListScorer.getScore(chRes2))
			if (parseTreeChunkListScorer.getScore(chRes1) > parseTreeChunkListScorer.getScore(chRes3))
				chRes = chRes1;
			else
				chRes = chRes3;
		else if (parseTreeChunkListScorer.getScore(chRes2) > parseTreeChunkListScorer.getScore(chRes3))
			chRes = chRes2;
		else
			chRes = chRes3;
	...
	public ParseTreeChunk generalizeTwoGroupedPhrasesRandomSelectHighestScore(ParseTreeChunk chunk1,
			ParseTreeChunk chunk2) {
		List<String> pos1 = chunk1.getPOSs();
		...
		for (int timesRun = 0; timesRun < timesRepetitiveRun; timesRun++) {
			...
			while (k1 < pos1.size() && k2 < pos2.size()) {
				// first check if the same POS
				String sim = posManager.similarPOS(pos1.get(k1), pos2.get(k2));
				String lemmaMatch = lemmaFormManager.matchLemmas(null, chunk1.getLemmas().get(k1),
						chunk2.getLemmas().get(k2), sim);
				...
				if ((sim != null) && (lemmaMatch == null || (lemmaMatch != null && !lemmaMatch.equals("fail")))) {
					...
					commonPOS.add(pos1.get(k1));
					if (chunk1.getLemmas().size() > k1 && chunk2.getLemmas().size() > k2 && lemmaMatch != null) {
