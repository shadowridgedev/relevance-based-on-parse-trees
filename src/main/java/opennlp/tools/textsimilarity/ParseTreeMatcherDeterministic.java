...
public class ParseTreeMatcherDeterministic {
	...
	/**
	 * key matching function which takes two phrases, aligns them and finds a
	...
	public List<ParseTreeChunk> generalizeTwoGroupedPhrasesDeterministic(ParseTreeChunk chunk1, ParseTreeChunk chunk2) {
		...
		for (int i = 1; i < occur1.size(); i++) {
			...
			if (occur1.get(i) > occur1.get(i - 1) && occur2.get(i) > occur2.get(i - 1))
		...
		for (List<int[]> occur : overlapsPlaus) {
			...
			while (k1 <= k1max && k2 <= k2max) {
				...
				String lemmaMatch = lemmaFormManager.matchLemmas(ps, lem1.get(k1), lem2.get(k2), sim);
				if ((sim != null) && (lemmaMatch == null || (lemmaMatch != null && !lemmaMatch.equals("fail")))) {
					...
					if (lemmaMatch != null) {
						...
						if (k1 == occr1.get(ov1) && k2 == occr2.get(ov2))
							bReachedCommonWord = true; // now we can have
														// different increment
														// opera
						else {
							if (occr1.size() > ov1 + 1 && occr2.size() > ov2 + 1 && k1 == occr1.get(ov1 + 1)
									&& k2 == occr2.get(ov2 + 1)) {
								ov1++;
								ov2++;
							...
							// else
							// System.err.println("Next match reached
							// '"+lemmaMatch+
							// "' | k1 - k2: "+k1 + " "+k2 +
							// "| occur index ov1-ov2 "+
							// ov1+" "+ov2+
							// "| identified positions of match: occr1.get(ov1)
							// - occr2.get(ov1) "
							// +
						...
						commonLemmas.add("*");
					} // the same parts of speech, proceed to the next word in
						// both
						// expressions
					k1++;
					k2++;
					...
					// different parts of speech, jump to the next identified
					// common word
					ov1++;
					ov2++;
					...
					int kk1 = occr1.get(ov1) - 2, // new positions of iterators
							kk2 = occr2.get(ov2) - 2;
					int countMove = 0;
					while ((kk1 < k1 + 1 || kk2 < k2 + 1) && countMove < 2) { // if
																				// it
																				// is
																				// behind
																				// current
	 ...
	 *         each resultant matched / overlapped phrase
	 */
	public List<List<ParseTreeChunk>> matchTwoSentencesGroupedChunksDeterministic(List<List<ParseTreeChunk>> sent1,
		...
		for (int comp = 0; comp < 2 && // just np & vp
			...
			for (ParseTreeChunk ch1 : sent1.get(comp)) {
				for (ParseTreeChunk ch2 : sent2.get(comp)) { // simpler version
					List<ParseTreeChunk> chunkToAdd = generalizeTwoGroupedPhrasesDeterministic(ch1, ch2);
					...
					// For generalized match not with orig sentences but with
					// templates
					// if (!LemmaFormManager.mustOccurVerifier(ch1, ch2,
					// chunkToAdd))
					// continue; // if the words which have to stay do not stay,
					// proceed
					// to other elements
