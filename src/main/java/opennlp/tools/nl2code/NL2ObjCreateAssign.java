...
public class NL2ObjCreateAssign extends NL2Obj {
	...
	public static String[] declarationStatesList = new String[] { "create", "assign", "set", };

	public static String[] dataTypesList = new String[] { "text", "double", "array", };

	public static String[] arrayElementList = new String[] { "first", "second", "third", "fourth" };

	public static String[] arrayElementListInsdex = new String[] { "0", "1", "2", "3" };
	...
	public ObjectPhraseListForSentence convertSentenceToControlObjectPhrase(String sentence) {
		...
		List<List<ParseTreeChunk>> lingPhrases = parser.formGroupedPhrasesFromChunksForSentence(sentence);
		...
		for (ParseTreeChunk verbChunk : actionWithObject) {
			...
			if (declarativeAction.equals("define")) {
				...
				if (classBeingDefined && Arrays.asList(dataTypesList).contains(dataType)
						&& verbChunk.getLemmas().get(2).toLowerCase().equals("attribute")) {
					op.setOperatorFor(
							dataType + " " + verbChunk.getLemmas().get(verbChunk.getLemmas().size() - 1).toLowerCase());
					classBeingDefined = true;
					break;
				}
				if (Arrays.asList(dataTypesList).contains(dataType)
						&& verbChunk.getLemmas().get(2).toLowerCase().equals("attribute")) {
					op.setOperatorFor(
							dataType + " " + verbChunk.getLemmas().get(verbChunk.getLemmas().size() - 1).toLowerCase());
					classBeingDefined = true;
				...
				if (verbChunk.getLemmas().get(1).toLowerCase().equals("array")) {
					...
					if (lems.contains("class")) {
						...
						op.setOperatorFor(lems.get(indClass + 1) + "[] "
								+ verbChunk.getLemmas().get(verbChunk.getLemmas().size() - 1).toLowerCase() + " = new "
								+ lems.get(indClass + 1) + "[" + arraySize + "]");
				...
				if (indQuantifier > -1 && (lems.get(indQuantifier + 1).equals("elements")
						|| lems.get(indQuantifier + 1).equals("members"))) {

					String arrayName = lems.get(lems.size() - 1);
					if (expression != null)
						op.setOperatorFor(
								"for(int i=0; i<" + arrayName + ".size(); i++) " + arrayName + "[i]." + expression);
					break;
				}
			}
			/*
			 * else { List<String> paramValues = verbChunk.getLemmas(),
			 * paramPOSs = verbChunk.getPOSs();
			 * 
			 * paramValues.remove(0); paramPOSs.remove(0); // the subject of a
			 * verb refers to the object try { String objectCandidatePOS =
			 * paramPOSs.get(paramValues.size()-1); if
			 * (objectCandidatePOS.startsWith("NN")){
			 * oPhrase.setObjectName(paramValues.get(paramValues.size()-1));
			 * paramValues.remove(paramValues.size()-1);
			 * paramPOSs.remove(paramPOSs.size()-1); } else if
			 * (paramPOSs.get(paramValues.size()-2).startsWith("NN")){
			 * oPhrase.setObjectName(paramValues.get(paramValues.size()-2));
			 * paramValues.remove(paramValues.size()-2);
			 * paramPOSs.remove(paramPOSs.size()-2); } } catch (Exception e) {
			 * e.printStackTrace(); } oPhrase.setParamValues(paramValues);
			 * oPhrase.setParamChunk(paramValues, paramPOSs);
			 * 
			 * // object name/instance if (oPhrase.getObjectName() == null){
			 * List<ParseTreeChunk> objectName = lingPhrases.get(0);
			 * ParseTreeChunk objNameChunk = objectName.get(0); if (
			 * objNameChunk.getPOSs().get(0).equals("NN")){
			 * oPhrase.setObjectName( objNameChunk.getLemmas().get(0)); } } }
			 * else { // verb = 'verify' attribute prep prep object
			 * 
			 * List<String> paramValues = verbChunk.getLemmas(), paramPOSs =
			 * verbChunk.getPOSs(); paramValues.remove(0); paramPOSs.remove(0);
			 * // we dont need 'verify'
			 * 
			 * // start with getting right-most noun as object String
			 * objectCandidatePOS = paramPOSs.get(paramValues.size()-1); if
			 * (objectCandidatePOS.startsWith("NN")){
			 * oPhrase.setObjectName(paramValues.get(paramValues.size()-1));
			 * paramValues.remove(paramValues.size()-1);
			 * paramPOSs.remove(paramPOSs.size()-1); } else if
			 ...
			 * oPhrase.setObjectName(paramValues.get(paramValues.size()-2));
			 * paramValues.remove(paramValues.size()-2);
			 * paramPOSs.remove(paramPOSs.size()-2); } // attempt to find
			 * attribute for(int i = paramValues.size()-1; i>=0; i--){ if
			 * (paramPOSs.get(i).equals("IN") || paramPOSs.get(i).equals("DT"))
			 * continue; else if
			 * (paramPOSs.get(i).startsWith("NN")||paramPOSs.get(i).startsWith(
			 * "JJ")||paramPOSs.get(i).startsWith("CD")){
			 * oPhrase.setMethod(paramValues.get(i)); paramValues =
			 * paramValues.subList(0, i-1); paramPOSs = paramPOSs.subList(0,
			 * i-1); oPhrase.setParamValues(paramValues);
			 * oPhrase.setParamChunk(paramValues, paramPOSs); break; } } }
			 * oPhrase.setOrigPhrase(verbChunk); oPhrase.cleanArgs(); //if
			 * (oPhrase.getMethod()!=null || oPhrase.getObjectName()!=null)
	...
	public static void main(String[] args) {
		...
		String[] text = new String[] { "Define a class and name it Employee. ",
				"Define text attribute and name it m_name. ", "Define double attribute and name it m_salary.",
				...
				// "Comment: We just started our small business company and
				// expect to hire 8 more people soon.",
