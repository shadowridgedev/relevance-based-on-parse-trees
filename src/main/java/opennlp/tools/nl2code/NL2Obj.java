...
public class NL2Obj {
	...
	public static String[] epistemicStatesList = new String[] { "select", "verify", "find", "start", "stop", "go",
			"check" };

	public static String[] instantiatedStatesList = new String[] { "get", "set" };
	public static String[] propStatesList = new String[] { "otherwise", };
		...
		for (ParseTreeChunk verbChunk : actionWithObject) {
			...
			if (verbChunk.getPOSs().get(0).startsWith("VB")) {
				...
				if (!isControlOp(methodOrControlOp)) {
					...
					List<String> paramValues = verbChunk.getLemmas(), paramPOSs = verbChunk.getPOSs();
					paramValues.remove(0);
					paramPOSs.remove(0); // we dont need 'verify'
					...
					for (int i = paramValues.size() - 1; i >= 0; i--) {
						if (paramPOSs.get(i).equals("IN") || paramPOSs.get(i).equals("DT"))
							continue;
						else if (paramPOSs.get(i).startsWith("NN") || paramPOSs.get(i).startsWith("JJ")
								|| paramPOSs.get(i).startsWith("CD")) {
				...
				oPhrase.cleanArgs();
				// if (oPhrase.getMethod()!=null ||
				// oPhrase.getObjectName()!=null)
	...
	/*
	 * public ObjectPhrase convertSentenceToControlObjectPhraseSingleObj(String
	 * sentence){ parser = ParserChunker2MatcherProcessor.getInstance();
	 * List<List<ParseTreeChunk>> lingPhrases =
	 * parser.formGroupedPhrasesFromChunksForSentence(sentence);
	 * 
	 * ObjectControlOp op = extractControlPart(lingPhrases, prevOp); prevOp =
	 * op;
	 * 
	 * ObjectPhrase oPhrase = new ObjectPhrase(); //start with verb phrases
	 * List<ParseTreeChunk> actionWithObject = lingPhrases.get(1);
	 * ParseTreeChunk verbChunk = actionWithObject.get(0); if
	 * (verbChunk.getPOSs().get(0).startsWith("VB")){
	 * oPhrase.setMethod(verbChunk.getLemmas().get(0)); List<String> paramValues
	 * = verbChunk.getLemmas(); paramValues.remove(0); // the subject of a verb
	 * refers to the object
	 * oPhrase.setObjectName(paramValues.get(paramValues.size()-1));
	 * paramValues.remove(paramValues.size()-1);
	 * oPhrase.setParamValues(paramValues); oPhrase.setOrigPhrase(verbChunk); }
	 * // object name/instance if (oPhrase.getObjectName() == null){
	 * List<ParseTreeChunk> objectName = lingPhrases.get(0); ParseTreeChunk
	 * objNameChunk = objectName.get(0); if (
	 * objNameChunk.getPOSs().get(0).equals("NN")){ oPhrase.setObjectName(
	 * objNameChunk.getLemmas().get(0)); } }
	 * 
	 * return oPhrase;
	 * 
	 * }
	...
	public static void main(String[] args) {
		...
		String[] text = new String[] { "Randomly select a pixel at an image.",
				...
				"Check that  the size of area is below the threshold. ", "Then go to 2",
