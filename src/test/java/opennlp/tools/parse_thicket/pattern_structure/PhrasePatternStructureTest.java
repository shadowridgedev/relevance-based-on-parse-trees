...
public class PhrasePatternStructureTest extends TestCase {
	...
	public void testNewsTest() {
		...
		// Description = "AMMAN, Jordan (AP) � A Syrian government official says
		// a car bomb has exploded in a suburb of the capital Damascus, killing
		// three people and wounding several others. The Britain-based Syrian
		// Observatory for Human Rights confirmed the Sunday explosion in
		// Jouber, which it said has seen heavy clashes recently between rebels
		// and the Syrian army. It did not have any immediate word on
		// casualties. It said the blast targeted a police station and was
		// carried out by the Jabhat al-Nusra, a militant group linked to
		// al-Qaida, did not elaborate.";
		// Description = "A car bombing in Damascus has killed at least nine
		// security forces, with aid groups urging the evacuation of civilians
		// trapped in the embattled Syrian town of Qusayr. The Syrian
		// Observatory for Human Rights said on Sunday the explosion, in the
		// east of the capital, appeared to have been carried out by the
		// extremist Al-Nusra Front, which is allied to al-Qaeda, although there
		// was no immediate confirmation. In Lebanon, security sources said two
		// rockets fired from Syria landed in a border area, and Israeli war
		// planes could be heard flying low over several parts of the country.";
		pt1 = ptBuilder.buildParseThicket(description);
		phrs1 = phraseBuilder.buildPT2ptPhrases(pt1);
		sent1GrpLst = lat.formGroupedPhrasesFromChunksForPara(phrs1);
		lat.AddIntent(sent1GrpLst, 0);
		...
		/*
		 * Set<Integer> intent = new HashSet<Integer>(); intent.add(0);
		 * intent.add(1); int gen = lat.GetMaximalConcept(intent,0);
		 * System.out.println("generator: " + gen); intent.clear();
		 * intent.add(0); intent.add(3);
		 * 
		 * lat.AddIntent(intent, 0);
		 * //System.out.println("after first addintent");
		 * //lat.printConceptByPosition(0); //lat.printConceptByPosition(1);
		 * intent.clear(); intent.add(0); intent.add(2); lat.AddIntent(intent,
		 * 0);
		 * 
		 * intent.clear(); intent.add(1); intent.add(2);
		 * 
		 * lat.AddIntent(intent, 0); intent.clear(); intent.add(tes1);
		 * intent.add(2); intent.add(3); lat.AddIntent(intent, 0);
		 * lat.printLattice(); lat.printLatticeStats();
		 */
	}
}