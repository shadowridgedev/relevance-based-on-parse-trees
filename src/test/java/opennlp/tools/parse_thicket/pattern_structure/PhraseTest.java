...
public class PhraseTest {
	...
	public static void main(String[] args) {
		 ...
		 * PhrasePatternStructureExtended(0,0); LinkedHashSet<Integer> obj =
		 * null; obj = new LinkedHashSet<Integer>(); obj.add(0);
		 * ps.AddIntent(chunks1, obj, 0); obj = new LinkedHashSet<Integer>();
		 * obj.add(1); ps.AddIntent(chunks2, obj, 0); obj = new
		 * LinkedHashSet<Integer>(); obj.add(2); ps.AddIntent(chunks3, obj, 0);
		 * ps.logStability(); ps.printLatticeExtended();
		 * 
		 * int [][] binaryContext = ps.toContext(3); for (int i = 0; i <
		 * binaryContext.length; i++ ){
		 * System.out.println(Arrays.toString(binaryContext[i])); }
		 * 
		 * ConceptLattice new_cl = new ConceptLattice(binaryContext.length,
		 * binaryContext[0].length, binaryContext,true);
		 * new_cl.printLatticeStats(); new_cl.printLatticeFull();
		 */

		/*
		 * // TEST 2 QUERY NEWS ParserChunker2MatcherProcessor chunk_maker =
		 * ParserChunker2MatcherProcessor.getInstance();
		 * List<List<ParseTreeChunk>> chunks = null; BingWebQueryRunner bq = new
		 * BingWebQueryRunner();
		 * 
		 * String q = ""; // q = "barack obama"; // q = "lady gaga"; q =
		 * "angela merkel"; // q = "putin"; ArrayList <HitBase> hb =
		 * (ArrayList<HitBase>) bq.runSearch(q, 10);
		 * PhrasePatternStructureExtended ps = new
		 * PhrasePatternStructureExtended(0,0); int cnt = 0; for (HitBase news:
		 * hb){ LinkedHashSet<Integer> obj = null; obj = new
		 * LinkedHashSet<Integer>(); obj.add(cnt); chunks =
		 * chunk_maker.formGroupedPhrasesFromChunksForPara(news.getDescription()
		 * ); System.out.println(chunks); ps.AddIntent(chunks,obj, 0); cnt++; }
		 * 
		 * ps.logStability(); System.out.println("LATTICE");
		 * ps.printLatticeExtended();
		 * 
		 * int [][] binaryContext = ps.toContext(cnt); for (int i = 0; i <
		 * binaryContext.length; i++ ){
		 * System.out.println(Arrays.toString(binaryContext[i])); }
		 * 
		 * ConceptLattice new_cl = new ConceptLattice(binaryContext.length,
		 ...
		 * new_cl.printLatticeStats(); new_cl.printLatticeFull();
		 * 
		 * FcaWriter wr = new FcaWriter(); wr.WriteAsCxt("res.cxt", new_cl);
		 * 
		 * System.out.println("Extent PS "+ps.conceptList.size()); //for (int i
		 * = 0; i<ps.conceptList.size();i++){ //
		 * System.out.println(ps.conceptList.get(i).extent); //}
		 * System.out.println("Extent CL "+new_cl.getLattice().size()); //for
		 * (int i = 0; i<new_cl.getLattice().size();i++){ //
