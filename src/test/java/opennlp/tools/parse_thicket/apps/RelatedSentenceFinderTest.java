...
public class RelatedSentenceFinderTest extends TestCase {
	...
	public void testAugmentWithMinedSentencesAndVerifyRelevanceTest() {
		...
		// assertTrue(result.getFragments().get(0).getFragment().indexOf("Swiss
		// Patent Office")>-1);
	}
	...
	public void testBuildParagraphOfGeneratedTextTest() {
		...
		input.setAbstractText(
				"Albert Einstein was a German-born theoretical physicist who developed the general theory of relativity, one of the two pillars of modern physics (alongside ...");
		input.setUrl("http://en.wikipedia.org/wiki/Albert_Einstein");
		input.setTitle("Albert Einstein - Wikipedia, the free encyclopedia");
		HitBase result = finder.buildParagraphOfGeneratedText(input, "Albert Einstein", new ArrayList<String>());
		System.out.println(result.toString());
		assertTrue(result.getOriginalSentences() != null);
		assertTrue(result.getOriginalSentences().size() > 0);
		assertTrue(result.getFragments().size() > 0);
	...
	public void testBuildParagraphOfGeneratedTextTestBio1() {
		...
		HitBase result = finder.buildParagraphOfGeneratedText(input, "applications of Einstein theories ",
	...
	/*
	 * public void testBuildParagraphOfGeneratedTextTestBio2(){ HitBase input =
	 * new HitBase(); input.
	 * setAbstractText("The theory of relativity is a beautiful example of  ..."
	 * ); input.setUrl("https://en.wikiquote.org/wiki/Albert_Einstein");
	 * input.setTitle("Albert Einstein"); HitBase result =
	 * finder.buildParagraphOfGeneratedText(input, "beautiful example of", new
	 * ArrayList<String>()); System.out.println(result.toString());
	 * assertTrue(result.getOriginalSentences()!=null);
	 * assertTrue(result.getOriginalSentences().size()>0);
	 * assertTrue(result.getFragments().size()>0);
	 * assertTrue(result.getFragments().get(0).getFragment().indexOf(
	 * "relativity")>-1); }
	 * 
	 * public void testBuildParagraphOfGeneratedTextTestBio3(){ HitBase input =
	 * new HitBase(); input.
	 * setAbstractText("I cannot conceive of a god who rewards and punishes his creatures or has a will of the kind that we experience  ..."
	 * ); input.setUrl("http://www.ldolphin.org/einstein.html");
	 * input.setTitle("Some Quotations of ALBERT EINSTEIN (1879-1955)"); HitBase
	 * result = finder.buildParagraphOfGeneratedText(input,
	 * "cannot conceive a god", new ArrayList<String>());
	 * System.out.println(result.toString());
	 * assertTrue(result.getOriginalSentences()!=null);
	 * assertTrue(result.getOriginalSentences().size()>0);
	 * assertTrue(result.getFragments().size()>0);
	 * assertTrue(result.getFragments().get(0).getFragment().
	 * indexOf("cannot conceive")>-1); }
	 * 
	 * 
	 * public void testBuildParagraphOfGeneratedTextTestBio4(){ HitBase input =
	 * new HitBase(); input.
	 * setAbstractText(" In 1905 our view of the world was changed dramatically and ..."
	 * ); input.setUrl(
	 * "http://philosophynow.org/issues/93/Albert_Einstein_1879-1955");
	 * input.setTitle("ALBERT EINSTEIN (1879-1955)"); HitBase result =
	 * finder.buildParagraphOfGeneratedText(input, "view of the world", new
	 * ArrayList<String>()); System.out.println(result.toString());
	 * assertTrue(result.getOriginalSentences()!=null);
	 * assertTrue(result.getOriginalSentences().size()>0);
	 * assertTrue(result.getFragments().size()>0);
	 * assertTrue(result.getFragments().get(0).getFragment().
	 * indexOf("view of the world")>-1); }
	 */

}

// [Albert Einstein (/�lbrt anstan/; German. albt antan ( listen); 14 March 1879
// 18 April 1955) was a German-born theoretical physicist who developed the
// general theory of relativity, one of the two pillars of modern physics
// (alongside quantum mechanics). 2 3 While best known for his massenergy
// equivalence formula E = mc2 (which has been dubbed "the world's most famous
// equation"), 4 he received the 1921 Nobel Prize in Physics "for his services
// to theoretical physics, and especially for his discovery of the law of the
// photoelectric effect". 5 The latter was pivotal in establishing quantum
// theory. nullNear the beginning of his career, Einstein thought that Newtonian
// mechanics was no longer enough to reconcile the laws of classical mechanics
// with the laws of the electromagnetic field. This led to the development of
